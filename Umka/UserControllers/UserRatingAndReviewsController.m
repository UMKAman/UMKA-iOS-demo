//
//  UserRatingAndReviewsController.m
//  Umka
//
...
#import "UserRatingAndReviewsController.h"
#import "ReviewModel.h"
#import "ReviewCell.h"
#import "RatingCell.h"
#import "RatingModel.h"

@interface UserRatingAndReviewsController ()
...
@implementation UserRatingAndReviewsController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.segmentControl = [[UISegmentedControl alloc] initWithItems:@[NSLocalizedString(@"Отзывы",@""),NSLocalizedString(@"Рейтинг",@"")]];
...
- (void)loadMasterInfo{
    [[ApiManager new] getReviews:[NSNumber numberWithInteger:[UmkaUser masterID]] completition:^(id response, NSError *error) {
...
{
    self.isRating = segmentControl.selectedSegmentIndex==1;
    [self.tableView reloadData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    else
    {
        ...
        label.text = NSLocalizedString(@"Еще нет отзывов", @"");
        return cell;
    }
    }
    else
